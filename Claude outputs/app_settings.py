...
Every field's default matches this app's actual current hardcoded behavior
(retry_count=2, num_steps=16, 24kHz/mono audio, etc.), so a customer who never
opens Settings gets identical behavior to before this endpoint existed.
...
class AppSettings(BaseModel):
    ...
    # --- Audio ---
    # "wav", "mp3", or "wav+mp3" - matches TTSRequest.format handling in
    # backend/services/tts_service.py (wav+mp3 means: submit as wav, service
    # also writes an mp3 alongside).

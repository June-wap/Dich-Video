...
class AppSettingsService:
    ...
    def resolve_retry_count(self) -> int:
        # A truly untouched install (nobody has ever opened Settings, so the
        # "app_settings" key has never been written at all) must reproduce the
        # exact legacy single-attempt behavior - not silently jump every fresh
        # customer to the schema's newer retry_count=2 default the instant
        # AppSettingsService exists. That invariant only starts once the user
        # has saved *something* from Settings at least once - see this
        # module's schema docstring ("identical to before this endpoint
        # existed"). `default=None` (vs `default={}`) is what lets us tell
        # "never saved" apart from "saved, but retry_count itself is missing
        # or corrupt" (the latter legitimately falls back to the schema's 2).
    ...
    def resolve_silence_trim(self) -> bool:
        ...
        resolve_retry_count()/resolve_num_steps()'s same "never saved yet"
        handling above - the schema's own default (True) only takes effect
        once the user has saved Settings at least once.
        """
        stored = self._db.get_setting(SETTINGS_KEY, default=None)
        if stored is None:
            return False
        value = stored.get("silence_trim")
    ...
    def resolve_num_steps(self) -> int | None:
        ...
        return value if isinstance(value, int) and 8 <= value <= 32 else None
